( function e( t, n, r ) {
		...
		module.exports = panels.view.dialog.extend( {
			...
			renderHistoryEntries: function () {
				...
				if ( this.currentEntry.get( 'data' ) !== this.revertEntry.get( 'data' ) || this.entries.models.length > 0 ) {
					$( this.historyEntryTemplate( {
							title: panelsOptions.loc.history.revert,
							count: 1
						} ) )
				...
				$( this.historyEntryTemplate( {
						title: panelsOptions.loc.history[ 'current' ],
						count: 1
					} ) )
		...
		module.exports = panels.view.dialog.extend( {
			...
			displayImportExport: function () {
				...
				var uploader = new plupload.Uploader( {
					...
					filters: [ {
						title: panelsOptions.plupload.filter_title,
						extensions: 'json'
					} ],
								...
								.blur( function () {
									...
									timeout = setTimeout( function () {
										...
										rowPreview.find( '.preview-cell' ).each( function ( i, el ) {
											$( el ).animate( {
												'width': Math.round( thisDialog.row.cells[ i ] * 1000 ) / 10 + "%"
											}, 250 );
	...
	12: [ function ( require, module, exports ) {
		...
		"./dialog/widget": 9,
		"./dialog/widgets": 10,
		"./jquery/setup-builder-widget": 11,
		"./model/builder": 13,
		"./model/cell": 14,
		"./model/history-entry": 15,
		"./model/row": 16,
		"./model/widget": 17,
		"./utils/menu": 18,
		"./view/builder": 19,
		...
		module.exports = Backbone.Model.extend( {
			...
			loadPanelsData: function ( data ) {
				...
				_.each( rows, function ( row, i ) {
					...
					var newRow = builderModel.addRow( row, {
						noAnimate: true
					} );
	...
	14: [ function ( require, module, exports ) {
		module.exports = Backbone.Model.extend( {
			...
			clone: function ( row, cloneOptions ) {
				...
				cloneOptions = _.extend( {
					cloneWidgets: true
				}, cloneOptions );

				var clone = new this.constructor( this.attributes );
				clone.set( 'collection', row.cells, {
					silent: true
				} );
				clone.row = row;
				...
				if ( cloneOptions.cloneWidgets ) {
					...
					this.widgets.each( function ( widget ) {
						clone.widgets.add( widget.clone( clone, cloneOptions ), {
							silent: true
						} );
					} );
				}
	...
	16: [ function ( require, module, exports ) {
		module.exports = Backbone.Model.extend( {
					...
					if ( cells.length > this.cells.length ) {
						...
						_.each( this.cells.slice( cells.length, this.cells.length ), function ( cell ) {
							...
							for ( var i = 0; i < widgetsToMove.length; i++ ) {
								widgetsToMove[ i ].moveToCell( newParentCell, {
									silent: false
								} );
							}
			...
			clone: function ( builder, cloneOptions ) {
				...
				var clone = new this.constructor( this.attributes );
				clone.set( 'collection', builder.rows, {
					silent: true
				} );
				...
				if ( cloneOptions.cloneCells ) {
					// Clone all the rows
					this.cells.each( function ( cell ) {
						clone.cells.add( cell.clone( clone, cloneOptions ), {
							silent: true
						} );
					} );
				}
	...
	17: [ function ( require, module, exports ) {
		...
		module.exports = Backbone.Model.extend( {
			...
			clone: function ( cell, options ) {
				...
				clone.set( 'values', cloneValues, {
					silent: true
				} );
				clone.set( 'collection', cell.widgets, {
					silent: true
				} );
				clone.cell = cell;
	...
	19: [ function ( require, module, exports ) {
		...
		module.exports = Backbone.View.extend( {
			...
			onAddRow: function ( row, collection, options ) {
				...
				var rowView = new panels.view.row( {
					model: row
				} );
			...
			triggerYoastSeoChange: function () {
				if ( $( '#yoast_wpseo_focuskw_text_input' ).length ) {
					var element = document.getElementById( 'yoast_wpseo_focuskw_text_input' ),
						event;
		...
		module.exports = Backbone.View.extend( {
			...
			render: function () {
				...
				this.model.widgets.each( function ( widget ) {
					var widgetView = new panels.view.widget( {
						model: widget
					} );
		...
		module.exports = Backbone.View.extend( {
			...
			refreshWidgets: function () {
				...
				this.builder.$( '.so-row-container' ).each( function ( ri, el ) {
					...
					var sectionWrapper = $( thisView.sectionTemplate( {
							title: 'Row ' + ( ri + 1 )
						} ) )
